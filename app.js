...
async function demoInit() {
  const snap = await (await fetch('demo/snapshot.json?v=20260922', { cache: 'force-cache' })).json();   // bump v when the snapshot is rebuilt, or browsers keep the old copy
  S.snapshot = snap;
  ...
  S.lists = (snap.lists || []).map(l => ({ ...l })); S.listBills = (snap.listBills || []).map(x => ({ ...x })); loadFilters(); S.listFollowers = Object.fromEntries((snap.lists || []).map(l => [l.id, l.followers || 0]));
...
// My bills: the ones I own or follow, less the ones I muted (a hearing unmutes them on the server).
const isOwner = b => !!S.me && (S.assignments[b.id] || []).includes(S.me.id);
const isMuted = b => !!S.mutes?.has(b.id);
const isMine = b => !!S.me && (isOwner(b) || !!S.follows?.has(b.id)) && !isMuted(b);
// The first hearing still ahead, which is what keeps a bill from being muted.
const hearingAhead = b => S.hearings.filter(h => h.bill_id === b.id && h.status === 'scheduled' && new Date(h.scheduled_at) > Date.now()).sort((x, y) => x.scheduled_at.localeCompare(y.scheduled_at))[0];
...
function saveFilters() { try { localStorage.setItem(FILTER_KEY, JSON.stringify({ v: 2, pris: [...S.pris], poss: [...S.poss], stands: [...S.stands], camps: [...S.camps], lsts: [...S.lsts], riskF: S.riskF, hearF: S.hearF, tripleF: S.tripleF, aliveF: S.aliveF })); } catch {} }
function loadFilters() { try { S.showMonitor = !!localStorage.getItem('hiphi_showmon'); } catch {} try { const f = JSON.parse(localStorage.getItem(FILTER_KEY) || 'null'); if (!f) return;
  // before v2 the position filter had four grouped choices; "Support" meant every kind of support
  const GROUPED = { support: ['strongly_support', 'support', 'support_amend'], oppose: ['strongly_oppose', 'oppose'] };
  S.pris = new Set(f.pris || []); S.poss = new Set((f.poss || []).flatMap(v => f.v >= 2 ? [v] : GROUPED[v] || [v])); S.stands = new Set(f.stands || []); S.camps = new Set((f.camps || []).filter(id => S.campaigns.some(c => c.id === id)));
  S.lsts = new Set((f.lsts || []).filter(id => !S.lists || S.lists.some(l => l.id === id)));
  S.riskF = !!f.riskF; S.hearF = !!f.hearF; S.tripleF = !!f.tripleF; S.aliveF = !!f.aliveF; } catch {} }
function toggleFilter(spec) { const i = spec.indexOf(':'), k = i < 0 ? spec : spec.slice(0, i), raw = i < 0 ? '' : spec.slice(i + 1);
  ...
  saveFilters(); }
function clearFilters() { S.pris = new Set(); S.camps = new Set(); S.lsts = new Set(); S.poss = new Set(); S.stands = new Set(); S.tripleF = false; S.riskF = false; S.hearF = false; S.aliveF = false; S.stageF = ''; saveFilters(); }
function visibleBills() {
  const list = barBase().filter(b => passes(b));
  const [key, dir] = S.sort;
  // Dead bills always sort to the bottom, whichever column and direction (Nate, 9/18).
  return [...list].sort((a,b) => {
    const da = diedish(a) ? 1 : 0, db = diedish(b) ? 1 : 0; if (da !== db) return da - db;
...
const filterCount = () => S.pris.size + S.camps.size + S.lsts.size + S.poss.size + S.stands.size + (S.tripleF ? 1 : 0) + (S.riskF ? 1 : 0) + (S.hearF ? 1 : 0) + (S.aliveF && S.view !== 'dead' ? 1 : 0) + (S.stageF ? 1 : 0);
const activeFilters = () => [...facets().flatMap(g => g.opts.filter(([v]) => S[g.key].has(v)).map(([v, l]) => [`${g.key}:${v}`, g.label, l])), ...FLAGS.filter(([k]) => S[k] && !(k === 'aliveF' && S.view === 'dead')).map(([k, l]) => [k, '', l]), ...(S.stageF ? [['stageF', 'Stage', STAGE_LABEL[S.stageF] || S.stageF]] : [])];
...
const PAGE_GROUPS = [
  // Outreach is what the public sees and receives: the people, the lists they follow, the emails they get.
  // The weekly memo is the team's own report on its bills, so it lives with the bills (Nate, 9/18).
  { key: 'bills', label: 'Bills', icon: '▤', short: 'Bills', views: [['table', 'Tracked bills'], ['add', 'New bills'], ['dead', 'Did not advance'], ['memo', 'Weekly memo']] },
  { key: 'outreach', label: 'Outreach', icon: '☺︎', short: 'Reach', views: [['people', 'People'], ['lists', 'Lists'], ['emails', 'Emails']] },
...
// list bills.
const FILTER_VIEWS = ['portfolio', 'table', 'dead'];
function filterBarHTML() {
  ...
  const quick = [...(S.view === 'table' ? [['aliveF', 'Still alive', FLAGS[3][2], FLAGS[3][3], 'aliveF', 'alive']] : []), ['pris:1', 'P1', 'top priority', f => f.pri === 1, 'pris', 'p1'], ...FLAGS.slice(0, 2).map(([k, l, tip, test], i) => [k, l, tip, test, k, i ? 'hear' : 'risk']), ['stands:a', 'Needs a hearing', 'in committee with nothing scheduled', f => f.stand === 'a', 'stands', 'need']];
  const isOn = spec => { const [k, v] = spec.split(':'); return v === undefined ? !!S[k] : S[k].has(k === 'pris' ? Number(v) : v); };
  return `<div class="fbar">
      <select class="scopesel" id="scopesel" aria-label="Whose bills"><option value="me" ${S.owner === 'me' ? 'selected' : ''}>My bills · ${barBase('me').length}</option><option value="all" ${S.owner === 'all' ? 'selected' : ''}>Everyone · ${barBase('all').length}</option>${S.advocates.filter(a => a.is_active !== false && a.id !== S.me?.id).map(a => `<option value="${a.id}" ${S.owner === a.id ? 'selected' : ''}>${esc(a.full_name)}</option>`).join('')}</select>
      ...
      <div class="qwrap">
        ...
        ${groupMenuHTML(base)}${mutedMenuHTML()}
...
// Coalitions and lists, one tap from the bar (Nate, 9/18): the two ways the team groups bills, side by side.
// Each choice shows how many bills it leaves, and the menu stays open so several can be picked.
function groupMenuHTML(base) {
  const camps = S.campaigns.filter(c => S.camps.has(c.id) || base.some(b => (S.billCampaigns[b.id] || []).includes(c.id)));
  const lists = (S.lists || []).filter(l => S.lsts.has(l.id) || base.some(b => factsOf(b).lsts.includes(l.id)));
  if (!camps.length && !lists.length) return '';
  const picked = [...camps.filter(c => S.camps.has(c.id)).map(c => c.name), ...lists.filter(l => S.lsts.has(l.id)).map(l => l.title)];
  const opt = (key, id, label) => { const on = S[key].has(id), c = facetCount(base, key, f => f[key].includes(id));
...
// A joint referral ("HLT/HSH") is one stop heard by two committees at once: both chairs must agree to a
// hearing and both committees vote. Every lookup by committee code goes through codesOf so none is dropped.
const codesOf = code => String(code || '').split('/').map(c => c.trim()).filter(Boolean);
// "Health / Human Services & Homelessness". Not "and": the Senate has a committee called Health and Human Services.
const cmteName = code => codesOf(code).map(c => S.committees?.[c]?.name || c).join(' / ');
// ▶ the hearing's video: exact link if someone saved one, else the chamber's YouTube channel (joint committees share a chamber).
...
function chrome(inner) {
  ...
  return `${railHTML(freshTxt, stale)}
    <div class="top staff">
      ...
      <span class="sbox"><input type="text" class="qbox topq" placeholder="Search any bill…" value="${esc(S.q)}" aria-label="Search any bill"><button class="qclear" type="button" aria-label="Clear the search" title="Clear the search">✕</button></span>
      <span class="fresh ${SIDE_RAIL ? 'railhas' : ''}"${stale ? ' style="color:#C2483B;font-weight:600" title="The daily sync has not completed successfully recently - data may be stale"' : ''}>${SESSION_YEAR} session · ${(ld => ld ? `${ld.today ? 'day' : 'recess, day'} ${ld.day} of ${ld.of} · ` : '')(legislativeDay())}${S.bills.length} tracked · ${freshTxt}</span>
      <span class="who">${av(S.me)}<button id="logout">sign out</button></span>
    </div>
    <div class="controls">
      <span class="sbox"><input type="text" class="qbox rowq" placeholder="Search any bill…" value="${esc(S.q)}" aria-label="Search any bill"><button class="qclear" type="button" aria-label="Clear the search" title="Clear the search">✕</button></span>
      ${filterBarHTML()}
    </div>
...
// "waiting in HHS · Chair Rep. Takayama": the person to call when a bill is stuck. A joint stop has a chair
// per committee and either one can hold the bill, so both are named.
function chairOf(code) {
  const cs = codesOf(code), legs = legsOf(code);
  const one = c => { const lg = legs.find(m => m.roles[c] === 'chair')?.l;
    if (lg) return `<a class="chairmail" data-leg="${lg.id}" onclick="event.stopPropagation()" title="Open the profile">${esc(legTitle(lg))} ${esc((lg.sort_name || '').split(',')[0] || lg.name.split(' ').pop())}</a>`;
    const m = chairMail(c); return m ? `<a class="chairmail" href="mailto:${esc(m.email)}" onclick="event.stopPropagation()" title="${esc(m.email)}">${esc(m.who)}</a>` : ''; };
  const found = cs.map(c => [c, one(c)]).filter(([, x]) => x);
  if (!found.length) return '';
  return cs.length === 1 ? ` · Chair ${found[0][1]}` : ` · Chairs ${found.map(([c, x]) => `${x} (${esc(c)})`).join(' and ')}`;
}
...
function renderPortfolio(list) {
  ...
  const actRow = ({ mine, b, t, until, ask, ctx, note, btn, extra = '' }) => `
    <div class="prow arow ${mine ? 'mine' : 'open srow'} ${t && t !== Infinity && t - now < 864e5 ? 'urgent' : ''}" data-bill="${b.id}">
      ${clock(t, until)}
  ...
  for (const f of (S.followups || []).filter(f => !f.done_at && f.advocate_id === S.me?.id)) { const p = f.person || personById(f.person_id); if (!p) continue; const t = f.due ? new Date(f.due + 'T17:00:00-10:00').getTime() : Infinity;
    merged.push({ mine: true, t, pri: 1, html: `<div class="prow arow mine ${t !== Infinity && t - now < 864e5 ? 'urgent' : ''}" data-personopen="${p.id}">${clock(t, 'to do')}<div class="amain"><div class="aask">${esc(f.what)} · <b class="abill">${esc(personName(p))}</b></div><div class="actx">follow-up${p.phone ? ` · ${esc(p.phone)}` : ''} · ${esc(p.email)}</div></div><div class="abtn"><button class="btn sm">Open</button></div></div>` }); }
  ...
  const waitingHtml = `<div class="actlist">
      <div class="acth">${subjectName} <span class="cnt">${mineItems.length}</span></div>${colHtml('mine', mineItems, 'Nothing is waiting on you. 🤙 Steps land here on their own: a draft is created about an hour after the Capitol posts a hearing notice, then it moves through review, approval and filing.')}
      <details class="actopen" id="fold-actopen" ${(S.folds ??= loadFolds()).actopen ? 'open' : ''}><summary class="acth open">Open to anyone <span class="cnt">${openItems.length}</span><small>anyone can take these</small></summary>${colHtml('open', openItems, 'Nothing unclaimed right now. Things that anyone can pick up appear here: a bill with no draft, a draft written for an older version, a P1 stuck without a hearing, a hearing with no public ask.')}</details>
      ${waitingOthers.length ? `<details class="actopen actothers" id="fold-actothers" ${(S.folds || {}).actothers ? 'open' : ''}><summary class="acth open">Waiting on others <span class="cnt">${waitingOthers.length}</span><small>the team’s open testimony steps</small></summary>${waitingOthers.slice(0, 12).map(waitRow).join('')}${waitingOthers.length > 12 ? `<div class="pempty">…and ${waitingOthers.length - 12} more</div>` : ''}</details>` : ''}
    </div>`;
  ...
  const waitPanel = `<div class="panel sec-wait" id="pf-wait">${waitHead}${waitingHtml}</div>`;
  ...
  const todayStrip = mobile && (todayHearings.length || dueToday.length) ? `<div class="todaystrip"><div class="th"><b>Today</b> · ${todayHearings.length} hearing${todayHearings.length === 1 ? '' : 's'}${dueToday.length ? ` · <span class="hot">${dueToday.length} testimony due</span>` : ''}</div>
      ${todayHearings.map(h => { const b = bill(h.bill_id); if (!b) return ''; const att = attendees(h); return `<div class="trow2" data-bill="${b.id}"><span class="tt">${new Date(h.scheduled_at).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', timeZone: 'Pacific/Honolulu' })}</span><span class="tb"><b>${esc(billNum(b))}</b> ${esc(h.committee)} · ${esc(roomShort(h.room))}${att.length ? ` · ${att.map(a => esc(a.initials)).join('/')} attending` : ATTEND_ASKS ? ' · <span class="hot">no one attending</span>' : ''}</span></div>`; }).join('')}
  ...
  return head(dashTitle(), `${today} · ${stripShort}${nextDl}`) + firstRunHTML() + banner + todayStrip + `
    ...
    ${board.html ? foldable('board', `<span class="bsumtitle">Where every bill stands</span><span class="foldsub">${board.a.length} need a hearing · ${board.b.length} scheduled · ${board.c.length} through committee</span>`, '', board.simpleHtml, false, true) : ''}
`;
}

// Bills > Did not advance (moved off the home page, 9/18): the bills that missed a deadline, were deferred or failed
// a vote this session, with why each one stopped. Follows the scope and filters like Tracked bills.
function renderDead(list) {
  const dead = list.filter(b => diedish(b)).sort((x, y) => (x.priority || 9) - (y.priority || 9) || x.bill_number.localeCompare(y.bill_number));
  return `<div class="dashhead"><h1>Did not advance</h1><span class="sub">${dead.length} bill${dead.length === 1 ? '' : 's'} stopped this session, and why</span></div>
    <div class="deadwrap"><div class="panel dead">${dead.length ? dead.map(b => `<div class="prow ${posCls(b)}" data-bill="${b.id}"><div class="pmain"><b>${esc(billNum(b))}</b> ${b.priority ? `<span class="pri">P${b.priority}</span>` : ''} <span class="chipx c-gray">${esc(b.died_at_stage ? (STAGE_LABEL[b.died_at_stage] || b.died_at_stage) : STAGE_LABEL[effStage(b)] || '')}</span><div class="pdesc">${esc(blurb(b, 120))}</div><div class="psmall">${whyDead(b)}</div></div>${owners(b)[0] ? av(owners(b)[0], 'avatar sm') : ''}</div>`).join('') : '<div class="pempty">No bill in this view has stopped. 🤙</div>'}</div></div>`;
}
function cell(b, c) {
  switch (c) {
...
function renderTable(list) {
  ...
  return `<div class="dashhead"><h1>Tracked bills</h1><span class="sub">${list.length} bill${list.length === 1 ? '' : 's'}${filterCount() ? ' match the filters' : ''}</span></div>` + banner + bulkBar() + billTable(list, ['sel','bill','owner','status','position','pri','last','pulse']);
}
...
function referralPath(b) {
  ...
  const line = (ch, list, leg) => list.length ? `<span class="refline"><span class="refch">${CHAMBER_NAME[ch]}</span>${list.map((c, i) => {
      ...
      return gl(esc(c), glossCommittee(c), `refstop ${here ? 'here' : past ? 'past' : ''}`); }).join('<span class="refarrow">→</span>')}</span>` : '';
...
function renderSettings() {
  ...
  const admin = !me.is_admin ? '' : `
    ...
    <section id="st-alerts">
      ...
      ${S.campaigns.map(c => `<label class="row chrow" data-chrow="${c.id}" ${c.slack_channel ? '' : 'hidden'}><span style="min-width:140px">${esc(c.name)}</span><input data-coal="${c.id}" value="${esc(c.slack_channel || '')}" placeholder="#channel-name"></label>`).join('')}
      ${S.campaigns.some(c => !c.slack_channel) ? `<label class="row"><span style="min-width:140px" class="tok">Add a channel for</span><select id="st-ch-add" style="width:auto"><option value="">choose a coalition…</option>${S.campaigns.filter(c => !c.slack_channel).map(c => `<option value="${c.id}">${esc(c.name)}</option>`).join('')}</select></label>` : ''}
  ...
  const SETUP_NAV = [['st-email', 'Email'], ['st-alerts', 'Hearing alerts'], ['st-ready', 'Session readiness'], ['st-coal', 'Coalitions'], ['st-counter', 'Committee map'], ['st-embed', 'Website embed'], ['st-import', 'Import'], ['st-conn', 'Connections']];
  if (S.view === 'setup') return `<div class="setupwrap">${admin ? `<nav class="setupnav" aria-label="Sections">${SETUP_NAV.map(([id, l]) => `<a data-jump="${id}">${l}</a>`).join('')}</nav>` : ''}<div class="settings"><h1>Session setup</h1><p class="tok" style="margin:-6px 0 14px">Admins only. Everything the season needs.</p>${admin || '<div class="pempty">Admins only.</div>'}</div></div>`;
  return `<div class="settings"><h1>My settings</h1><p class="tok" style="margin:-6px 0 14px">How the tracker reaches you.</p>${mine}</div>`;
}
function wireSettings() {
  ...
  if (conn) {
    ...
    $('#st-ch-add') && ($('#st-ch-add').onchange = () => { const id = $('#st-ch-add').value; if (!id) return; const row = document.querySelector(`[data-chrow="${id}"]`); if (row) { row.hidden = false; row.querySelector('input').focus(); } $('#st-ch-add').querySelector(`option[value="${id}"]`).remove(); $('#st-ch-add').value = ''; });
...
const SHORTCUTS = [
  ...
  ['1 – 5', 'Bill tabs: Details (with the timeline), Pathway, Team, Public, Chat (with the team note)'], ['n / p', 'Next / previous week on the calendar'],
...
function renderPeople() {
  ...
  return `<div class="peoplewrap">
    <div class="dashhead"><h1>People</h1><span class="sub">Everyone HIPHI knows, and what they follow and do.</span></div>
...
function personDrawerHTML(p) {
  ...
  const details = ed ? `<div class="pdform">
      ...
      <label class="row legbox"><span>Districts</span><span class="grow"><input id="pd-addr" value="${esc(a.q ?? '')}" placeholder="Type an address to find the districts — the address itself is not kept" autocomplete="off">${a.results?.length && !a.picked ? `<div class="legsug">${a.results.map((x, i) => `<button data-pdpick="${i}"><span class="sk">📍</span>${esc(x.label)}</button>`).join('')}</div>` : ''}</span></label>
      <div class="tok pdd"><b id="pd-dist">${a.picked ? (a.sd ? `Senate ${a.sd} · House ${a.hd} · ${esc(islandOf(a.sd) || '')}` : 'unknown for that address') : p.senate_district ? `Senate ${p.senate_district} · House ${p.house_district} · ${esc(p.island || '')}` : 'unknown'}</b>${p.senate_district || a.picked ? ' · <button class="linkbtn" id="pd-distclear" type="button">clear</button>' : ''} <span class="muted">Only the districts are stored; street addresses stay private to the person.</span></div>
      ...
      ${def2('Where', whereOf(p) || '<span class="muted">unknown</span>')}
...
// ---- first-run checklist: five things a new teammate does once. Shown for the first 45 days (and in the sandbox),
// ticks itself as they do each one, and goes away for good when finished or dismissed.
const FR_KEY = DEMO ? 'hiphi_firstrun_demo' : 'hiphi_firstrun';
const frGet = () => { try { return JSON.parse(localStorage.getItem(FR_KEY) || '{}'); } catch { return {}; } };
const frSet = patch => { const o = { ...frGet(), ...patch }; try { localStorage.setItem(FR_KEY, JSON.stringify(o)); } catch {} return o; };
function firstRunMark() { const o = frGet(); if (o.dismissed) return; const p = {}; if (S.view === 'settings' && !o.settings) p.settings = 1; if (S.view === 'inbox' && !o.inbox) p.inbox = 1; if (S.view === 'help' && !o.help) p.help = 1; if (S.drawerBill && !o.bill) p.bill = 1; if (Object.keys(p).length) frSet(p); }
function firstRunHTML() {
  const o = frGet(), me = S.me; if (!me || o.dismissed) return '';
  const isNew = DEMO || (me.created_at && Date.now() - new Date(me.created_at) < 45 * 864e5); if (!isNew) return '';
  const mine = S.bills.filter(b => (S.assignments[b.id] || []).includes(me.id) || (S.followersBy?.[b.id] || []).includes(me.id)).length;
  const steps = [
    [!!o.settings, 'Check how the tracker reaches you', 'Slack messages and testimony reminders, in My settings', 'settings'],
    [mine >= 3, `Follow three bills you care about${mine ? ` · ${Math.min(mine, 3)} of 3` : ''}`, 'open a bill and press Follow; owned bills count', 'table'],
    [!!o.bill, 'Open one bill and read its Next box', 'it says what the bill needs and by when', null],
    [!!o.inbox, 'Look at your Inbox', 'the red number counts only what needs you', 'inbox'],
...
// Every page opens the same way: the title, then the page tabs, then the filters, then the content.
function arrangeHeader() {
  const app = document.getElementById('app');
  const h1 = [...app.querySelectorAll('h1')].find(h => !h.closest('.rail, .top, .drawer, .scrim'));
  if (!h1) return;
  const head = h1.closest('.dashhead') || (h1.nextElementSibling?.matches('p.tok') ? h1.nextElementSibling : h1);
  const controls = app.querySelector(':scope > .controls');
  if (controls && h1.closest('.dashhead') && FILTER_VIEWS.includes(S.view)) controls.before(head);
  const tabs = app.querySelector('.pagetabs');
  if (tabs) head.after(tabs);
}
// Rows that open something on click can also be reached with Tab and opened with Enter or Space; fields without a
// visible label get a spoken one.
const ROW_SEL = '[data-bill]:not(a):not(button):not(select):not(input), [data-personopen]:not(button), [data-inbox], [data-idup], [data-alertopen]:not(button), [data-leg]:not(button), [data-view].frrow, [data-pwfold]';
const FIELD_NAMES = { 'leg-q': 'Search legislators by name, town, district or address', 'leg-ch': 'Chamber', 'leg-cm': 'Committee', 'pp-q': 'Search people', 'pp-island': 'Island', 'pp-sd': 'Senate district', 'pp-hd': 'House district', 'pp-camp': 'Coalition', 'pp-list': 'List', 'pp-tag': 'Tag', 'pp-int': 'Interest', 'pp-acct': 'Accounts or contacts', 'pp-active': 'Active in', 'pp-sort': 'Sort', 'addq': 'Search every introduced bill', 'help-q': 'Search Help', 'memo-who': 'Whose bills', 'memo-coal': 'Coalition', 'em-new': 'New alert about', 'inbox-q': 'Filter the Inbox', 'inbox-sort': 'Sort', 'st-mont': 'Morning reminder time', 'st-befh': 'Hours before the deadline', 'st-aftt': 'After-deadline reminder time', 'scopesel': 'Whose bills', 'stagef': 'Stage' };
function a11yPass() {
  document.querySelectorAll(ROW_SEL).forEach(el => { if (!el.hasAttribute('tabindex')) el.tabIndex = 0; if (!el.getAttribute('role') && el.tagName !== 'TR') el.setAttribute('role', 'button'); });
  document.querySelectorAll('input:not([type=hidden]):not([type=checkbox]):not([type=radio]), select, textarea').forEach(el => {
    if ((el.labels && el.labels.length) || el.getAttribute('aria-label')) return;
    const name = FIELD_NAMES[el.id] || el.placeholder || el.title || (el.tagName === 'SELECT' ? el.options[0]?.textContent : '');
    if (name) el.setAttribute('aria-label', name.replace(/…$/, ''));
  });
}
document.addEventListener('keydown', e => {
  if (e.key !== 'Enter' && e.key !== ' ') return;
  const el = e.target; if (!(el instanceof Element) || !el.matches(ROW_SEL)) return;
...
function renderHelp() {
  ...
  return `<div class="settings help"><h1>Help</h1>
    ...
    ${sec('dash', 'The dashboard', `
      ...
      ${def('News on your bills', 'Official actions (referrals, votes, hearings posted) are in the Inbox under Updates.')}
      ${def('Did not advance', 'Bills that missed a deadline, were deferred or failed a vote, with why: Bills, then the Did not advance tab.')}`)}
    ${sec('bill', 'A bill’s page', `
      ...
      ${def('Details', 'Committee and chair, referrals in both chambers, sponsors, companion, the official description and title, the video link, the Capitol source. At the bottom, the <b>Timeline</b> fold: every official action and everything the team logged.')}
      ...
      ${def('Team', 'Position, priority, owner, stage override, coalitions. Changes save as you make them.')}
      ...
      ${def('Chat', 'The pinned team note (never public, context that should outlive the thread), tasks, and the team thread on this bill. Owners, followers and anyone @mentioned get a Slack DM; new messages also land in the Inbox.')}
      ${def('Hover or tap', 'Stage names, committee codes and the priority badge explain themselves when you hover, focus or tap them.')}`)}
...
function renderInbox() {
  ...
  return `<div class="inbox">
    <div class="dashhead"><h1>Inbox</h1><span class="sub">What needs you, and news on your bills.</span></div>
    <div class="itabs">
...
function renderMemo() {
  ...
  return `<div class="memowrap">
    <div class="dashhead"><h1>Weekly memo</h1><span class="sub">A weekly summary of your bills, ready to copy.</span></div>
...
// Everyone who votes at a stop. On a joint stop a legislator on both committees is listed once, with a role
// per committee (roles) and their highest role (role, committee) for sorting.
...
function renderLegislators() {
  ...
  return `<div class="legwrap">
    <div class="dashhead"><h1>Legislators</h1><span class="sub">Every senator and representative.</span></div>
...
// long lists in a drawer: eight rows, the rest behind one tap
const foldRows = (rows, n = 8) => rows.length <= n + 3 ? rows.join('') : rows.slice(0, n).join('') + `<details class="morefold"><summary>Show all ${rows.length}</summary>${rows.slice(n).join('')}</details>`;
function legDrawerHTML(l) {
  ...
  const inCommittee = S.bills.filter(b => b.tracked !== false && b.position !== 'monitor' && !diedish(b) && (() => { const st = stopOf(b); return st.phase === 'committee' && st.committee && mine.some(m => codesOf(st.committee).includes(m.committee)); })()).slice(0, 12);
  ...
  <div class="drawer v2 legdrawer">
    ...
    <div class="dbody">
      ...
      <div class="sec">Our bills in their committees <span class="tok">live, non-monitor · ${inCommittee.length}</span></div>${foldRows(inCommittee.map(billLine)) || '<p class="muted">None right now.</p>'}
      <div class="sec">Bills of ours they introduced <span class="tok">${sponsored.length}</span></div>${foldRows(sponsored.map(billLine)) || '<p class="muted">None on the tracker.</p>'}
...
function pathwayHTML(b) {
  ...
  // On a joint stop a leadership role names its committee ("Chair, HLT"), and someone on only one of the
  // two committees says which ("HSH only"); sitting on both is the normal case and needs no words.
  const roleText = (m, joint) => { const word = r => r === 'chair' ? 'Chair' : 'Vice Chair';
    if (!joint) return m.role === 'member' ? '' : word(m.role);
    const lead = Object.entries(m.roles).filter(([, r]) => r !== 'member').map(([c, r]) => `${word(r)}, ${c}`);
    return lead.length ? lead.join(' · ') : Object.keys(m.roles).length === 1 ? `${Object.keys(m.roles)[0]} only` : ''; };
  ...
  const stopBlock = (s, i) => { const members = legsOf(s.committee), joint = codesOf(s.committee).length > 1;
    ...
    return `<div class="pwstop pws-${s.state}">
      <div class="pwh" data-pwfold="${key}" role="button" tabindex="0" aria-expanded="${open}"><span class="pwst" aria-hidden="true">${s.state === 'passed' ? '✓' : s.state === 'current' ? '●' : '○'}</span>
        <span class="pwt"><b>${esc(cmteName(s.committee))}</b><span class="pwmeta"><span class="code">${esc(s.committee)}</span>${joint ? ' · joint hearing' : ''} · ${CHAMBER_NAME[s.chamber]} stop ${s.stop} of ${s.of} · ${label}</span></span>
        ${members.length && s.state !== 'passed' ? `<span class="pwtally">${count(members)}</span>` : ''}<span class="lchev" aria-hidden="true">${open ? '▴' : '▾'}</span></div>
      ${s.state === 'passed' && !open ? `<div class="pwpassed">${chairs.map(m => `<a data-leg="${m.l.id}">${esc(legTitle(m.l))} ${esc(m.l.name)}</a>, chair${joint ? ` of ${esc(m.committee)}` : ''}`).join('; ') || ''}</div>` : ''}
      ${open ? `<div class="pwbody">
        ${chairs.map(m => person(m, true, joint)).join('')}
        ${vices.map(m => person(m, true, joint)).join('')}
        ${rest.length ? `<div class="pwmembers">${rest.map(m => person(m, false, joint)).join('')}</div>` : ''}
...
function renderEmails() {
  ...
  return `<div class="emailswrap">
    <div class="dashhead"><h1>Emails to the public</h1><span class="sub">Action alerts to people who follow a bill or list.</span></div>
...
function renderTriage(embedded = false) {
  ...
  const chips = `<div class="tchips">
      ...
      <label class="row tmatched"><input type="checkbox" id="t-matched" ${t.matchedOnly ? 'checked' : ''}><span>suggestions only</span></label></div>`;
  ...
  const campName = t.camp ? (S.campaigns.find(x => x.id === t.camp)?.name || 'this coalition') : 'all coalitions';
  const left = t.rows ? t.rows.length : 0, total = left + t.done;
  const progress = t.rows === null ? '' : `<div class="tprog"><div class="tprogbar"><span style="width:${total ? Math.round(t.done / total * 100) : 0}%"></span></div>
      <span><b>${t.done}</b> decided this sitting · <b>${left}${left >= 400 ? '+' : ''}</b> left ${t.matchedOnly ? 'among the suggestions' : 'undecided'} for ${esc(campName)}</span></div>`;
  const rows = t.rows === null ? '<div class="pempty">Loading the queue…</div>'
    : t.rows.length ? t.rows.slice(0, t.show).map(row).join('') + (t.rows.length > t.show ? `<button class="pempty boardmore" id="t-more">Show ${Math.min(BATCH, t.rows.length - t.show)} more · ${t.rows.length - t.show} further down the queue</button>` : '')
...
function wireTriage() {
  ...
  document.querySelectorAll('.trow .tdesc').forEach(el => el.onclick = e => { e.stopPropagation(); el.classList.toggle('open'); });
  ...
  const act = async (id, fn) => { const r = (t.rows || []).find(x => x.id === id); if (!r) return;
    try { await fn(r); t.rows = t.rows.filter(x => x.id !== id); t.done = (t.done || 0) + 1; t.focus = Math.min(t.focus, Math.max(0, Math.min(t.rows.length, t.show || 25) - 1)); if (t.counts) { t.counts.undecided--; } rerenderKeep(); }
    catch (e) { toast(e.message, true); } };
  ...
  document.querySelectorAll('[data-tskip]').forEach(el => el.onclick = () => act(el.dataset.tskip, async r => { await DB.triageSkip(r); t.last = { ...r, tracked: false }; toastUndo(`${r.bill_number} skipped`, async () => { await DB.triageUndo(r); t.last = null; t.rows = null; t.done = Math.max(0, (t.done || 0) - 1); render(); }); }));
  document.querySelectorAll('[data-tundo]').forEach(el => el.onclick = async () => { const r = t.last; if (!r) return; try { await DB.triageUndo(r); t.last = null; t.rows = null; t.done = Math.max(0, (t.done || 0) - 1); render(); toast(`${r.bill_number} is back in the queue`); } catch (e) { toast(e.message, true); } });
...
function timelineHTML(tlIn) {
  ...
  tl = shown;
  return `<div class="tlcap">${tl.length} entr${tl.length === 1 ? 'y' : 'ies'}, newest first. Entries the team logged show who wrote them.${DEMO ? ` · sandbox is frozen at ${fmtDate(DEMO_ASOF, { year: 'numeric' })}, later actions are not here` : ''}</div>` +
    `<div class="tl">${shown.map(ev => {
    const team = ev.source === 'team', a = advocate(ev.advocate_id);
    // The Capitol dates its actions but gives no time (they arrive stamped 8:00 AM), and every one says
    // "Official action - House"; the date and the chamber are the facts, on one line.
    const ch = !team && /^Official action\s*-\s*(House|Senate)$/i.exec(ev.details || '');
    return `<div class="ev ${team?'team':''}">
      <div class="when">${team ? `${fmtDT(ev.occurred_at)} · <b>${esc(a ? a.full_name : 'Team')}</b>` : `${fmtDate(ev.occurred_at, { weekday: 'short', year: '2-digit' }).replace(/^(\w{3}),/, '$1')}${ch ? ` · ${ch[1]}` : ''}`}</div>
      <div class="what">${esc(ev.title)}</div>
      ${ev.details && !ch ? `<div class="who">${esc(ev.details)}</div>` : ''}
    </div>`; }).join('')}</div>`;
}
...
// The Capitol's words, explained where they appear (hover, keyboard focus, or a tap on a phone).
const STAGE_GLOSS = {
...
const glossStage = s => { const d = (DEADLINES[s] || []).map(([l, dt]) => `${l} ${fmtDate(dt + 'T12:00:00-10:00', { weekday: 'short', month: 'short' })}`).join(', '); return `${STAGE_GLOSS[s] || ''}${d ? ` Deadline: ${d}.` : ''}`; };
const glossCommittee = code => String(code || '').split('/').map(c => { const k = S.committees?.[c.trim()]; return k ? `${c.trim()}: ${k.name}${k.chair ? ` · chair ${k.chair}` : ''}` : ''; }).filter(Boolean).join(' — ');
const gl = (text, tip, cls = '') => tip ? `<span class="gl ${cls}" tabindex="0" data-gl="${esc(tip)}">${text}</span>` : (cls ? `<span class="${cls}">${text}</span>` : text);
const PRI_GLOSS = { 1: 'Priority 1: the team’s top tier. These lead every list and alert.', 2: 'Priority 2: active, behind the P1 bills.', 3: 'Priority 3: lowest tier we still take a position on.' };
...
function draftJourney(d) {
  // the highlighted step is the one waiting to happen: an approved draft waits to be filed
  const idx = { draft: 0, review: 1, second_review: 1, approved: 3, filed: 4 }[d.status] ?? 0, done = d.status === 'filed';
...
// "SAN BUENAVENTURA" -> "San Buenaventura", "LEE, M." -> "Lee, M.": capitals are hard to scan in a long list.
const sponsorName = n => String(n || '').toLowerCase().replace(/(^|[\s\-'.(])([a-z])/g, (m, a, c) => a + c.toUpperCase());
function drawerHTML(b) {
  ...
  if (dead) {
    ...
    nextHtml = ups.map(h => { const known = codesOf(h.committee).some(k => S.committees?.[k]); const dr = draftFor(b.id, h.committee); if (dr) seen.add(dr.id);
  ...
  const tab = (k, l) => `<button class="dtab ${open.tab === k ? 'on' : ''}" data-dtab="${k}" role="tab" aria-selected="${open.tab === k}">${l}</button>`;
  ...
  <div class="drawer v2">
    ...
    <div class="dbody">
      ...
      ${pane('details', `<div class="kv dkv">
          ...
          ${(h => { const v = h && streamOf(h); return v ? `<span class="k">Hearing video</span><span>${streamLink(h)}<span class="muted"> · ${v.auto ? 'found automatically' : v.exact ? 'link set by staff' : v.search ? 'searches the ' + esc(v.channel) + ' channel for this hearing' : esc(v.channel) + ' channel; the stream appears shortly before the start'}</span> <button class="draftbtn" data-setstream="${h.id}">${v.exact ? 'Change' : 'Paste a link'}</button></span>` : ''; })(ups[0])}
          ${ups.map(h => { const a = attendees(h), meIn = a.some(x => x.id === S.me?.id); return `<span class="k">Attending ${esc(h.committee)}</span><span>${a.length ? esc(a.map(x => x.full_name).join(', ')) + ' ' : '<span class="muted">No one yet </span>'}<button class="draftbtn" data-attend="${h.id}">${meIn ? 'Not attending' : 'I\u2019m attending'}</button></span>`; }).join('')}
          <span class="k">Links</span><span class="dlinks2"><a href="${esc(capitolUrl(b))}" target="_blank" rel="noopener">Capitol page ↗</a><button class="draftbtn" data-copylink="${esc(b.bill_number)}">Copy a link to this bill</button></span>
        </div>`)}
      ${pane('timeline', `${open.log ? `<div class="logform">
          <div class="typechips">${LOG_TYPES.map(([v,l]) => `<button data-lt="${v}" class="${S.logType===v?'on':''}">${l}</button>`).join('')}</div>
          <input id="d-ltitle" aria-label="What happened" placeholder="${S.logType==='testimony'?'e.g. Testimony submitted — Support (written + oral)':'What happened?'}">
          <textarea id="d-ldetails" aria-label="Details" placeholder="Details (optional)"></textarea>
          <button class="btn sm" id="d-log">Add to timeline</button> <button class="draftbtn" id="d-logclose">Cancel</button>
        </div>` : `<button class="morelink" id="d-logopen">+ Log an action or add a note</button>`}
...
function render() {
  ...
  const body = S.view === 'portfolio' ? renderPortfolio(list)
    ...
    : S.view === 'add' ? renderAdd()
    : S.view === 'dead' ? renderDead(list) : renderTable(list);
  const b = S.bills.find(x => x.id === S.drawerBill);
  const HELP_FOR = { portfolio: 'help-dash', inbox: 'help-pages', add: 'help-pages', dead: 'help-pages', triage: 'help-pages', table: 'help-pages', legislators: 'help-pages', people: 'help-pages', lists: 'help-public', emails: 'help-public', memo: 'help-pages', settings: 'help-auto', setup: 'help-where' };
...
function wire() {
  ...
  document.querySelectorAll('.qclear').forEach(el => el.onclick = () => { S.q = ''; render(); document.querySelector('.qbox.' + (S.qFocus || 'topq'))?.focus(); });
  document.querySelectorAll('[data-scopeall]').forEach(el => el.onclick = () => { S.owner = 'all'; render(); });
  ...
  document.querySelectorAll('[data-ft]').forEach(el => el.onclick = () => { S.grpMenuOpen = !!el.dataset.keepmenu; toggleFilter(el.dataset.ft); filterRender(); });
  $('.grpmenu') && $('.grpmenu').addEventListener('toggle', e => { S.grpMenuOpen = e.target.open; });
...
function wireDrawer() {
  ...
  // Mute works from the bill header and from the Muted menu in the filter bar, so it is wired with no bill open too.
  document.querySelectorAll('[data-mute]').forEach(el => el.onclick = async () => { const b = billById(el.dataset.mute); if (!b) return;
    const on = !isMuted(b), h = on && hearingAhead(b);
    if (h) { toast(`${billNum(b)} has a ${h.committee} hearing ${fmtDT(h.scheduled_at)}. You can mute it once that hearing is over.`, true); return; }
    try { await DB.mute(b.id, on); on ? toastUndo(`Muted ${billNum(b)}: off your dashboard and no alerts until it gets a hearing`, async () => { await DB.mute(b.id, false); rerenderKeep(); }) : toast(`${billNum(b)} is back on your dashboard`); rerenderKeep(); }
    catch (e) { toast(e.message, true); } });
  ...
  document.querySelectorAll('[data-campt]').forEach(el => el.onclick = async () => {
    ...
    try { await DB.toggleCampaign(b.id, id, on);
      S.drawerOpen.teamSaved = Date.now(); toast(on ? `Added to ${nm}` : `Removed from ${nm}`); render(); }
