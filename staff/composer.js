...
import { icon, btn, iconBtn, chip, empty, notice, toast, openSheet, closeSheet, confirmSheet, menuSheet } from './ui.js';
import { listById, listIcon, listRows, plural, afterClose, clip } from './lists.js';
...
function whoResults(q) {
  ...
  const bRows = bills.slice(0, t ? 8 : 6).map(b => { const h = hearingAhead(b); return opt('b:' + b.id, 'scroll-text', `<b>${esc(billNum(b))}</b> <span class="le-t">${esc(clip(b, 70))}</span>`, h ? `Hearing ${esc(fmtDT(h.scheduled_at))} · ${esc(h.committee)}` : 'No hearing yet'); });
...
function whoStep(c) {
  ...
  if (u.kind) {
    const locked = !!c.a.id;
    const title = u.b ? `<b>${esc(billNum(u.b))}</b> <span class="le-t">${esc(clip(u.b, 90))}</span>` : esc(u.name);
...
function writeStep(c) {
  ...
  return `<h2 class="le-sh">Write it</h2>
    ...
    <div class="le-savebar"><span class="meta" id="le-saved">${savedLine(c)}</span>${btn('Save draft', { kind: 'text', icon: 'check', attrs: { 'data-le': 'save' } })}</div>`;
}
const phLine = n => n ? `${icon('square-pen')}<span>${n === 1 ? 'One bit' : `${n} bits`} of text in brackets to replace.</span>` : '';
const savedLine = c => c.dirty || !c.a.id ? (c.a.id ? 'Changes not saved yet' : 'Not saved yet') : `Draft saved ${esc(ago(c.savedAt))}`;

function previewStep(c) {
  const a = previewOf(c);
  return `<h2 class="le-sh">Check it</h2>
    <div class="le-prevtop"><p class="le-aud">${icon('users')}${audHTML(c.a, 'goes')}</p>${canTest(c.a) ? btn('Send me a test', { kind: 'secondary', sm: true, icon: 'mail-check', attrs: { 'data-le': 'test' } }) : ''}</div>
    ${emailPreview(a)}`;
}
function submitStep(c) {
  const me = S.me || {}, others = approvers(me.id);
  return `<h2 class="le-sh">Send it for approval</h2>
    ...
    <p class="le-rule">${icon('user-check')}<span>An admin other than you approves it. Then you send it.</span></p>
    ${others.length ? `<p class="meta le-rule2">${esc(orJoin(others.map(x => first(x.id))))} ${others.length === 1 ? 'gets' : 'get'} a Slack message now.</p>`
      : notice('info', 'info', 'You are the only admin, so nobody can approve it yet. Every email needs an admin other than its writer.')}`;
}
...
function statusView(a) {
  ...
  else if (a.status === 'approved') line = `Approved${a.approved_by ? ' by ' + esc(first(a.approved_by)) : ''}. ${own ? 'Send it when you are ready.' : me.is_admin ? `${esc(who)} or an admin can send it.` : `Waiting for ${esc(who)} to send it.`}`;
  ...
  return `<div class="le-cmp le-status">
    <a class="le-deskback" href="#/outreach/emails" data-back>${icon('chevron-left')}<span>Emails</span></a>
    <div class="le-chead"><div><p class="le-eyebrow">${icon('mail')}Email to supporters</p><h1>${esc(a.subject || '(no subject)')}</h1></div>${more}</div>
    ${pausedNotice()}
    <section class="card le-state" aria-label="Where it stands">
...
function render(route) {
  ...
  const body = [whoStep, writeStep, previewStep, submitStep][c.step](c);
  return `<div class="le-cmp">
    <a class="le-deskback" href="#/outreach/emails" data-back>${icon('chevron-left')}<span>Emails</span></a>
    ...
    ${stepBar(c.step)}
    <section class="le-step">${body}</section>
  </div>`;
}
function bar(route) {
  if (!S.me) return '';
  if (route.id) {
    ...
    if (!canEdit(a)) {
      if (canApprove(a)) return `<div class="le-bar">${btn('Send back', { kind: 'secondary', icon: 'undo-2', attrs: { 'data-le': 'return' } })}${btn('Approve', { icon: 'check', attrs: { 'data-le': 'approve' } })}</div>`;
      if (canSend(a)) return `<div class="le-bar">${btn(audHTML(a, 'send'), { icon: 'send', attrs: { 'data-le': 'send' } })}</div>`;
      return '';
    }
  }
  ...
  return `<div class="le-bar">${backB}${nextB}</div>`;
}
...
function paintPH(ed) {
  if (!ed || !(window.CSS && CSS.highlights && typeof Highlight === 'function')) return;
  const ranges = [], w = document.createTreeWalker(ed, NodeFilter.SHOW_TEXT); let n;
...
function wireRTE(root, c) {
  ...
  const changed = () => {
    ...
    root.querySelector('#le-ph').innerHTML = phLine(phCount(c)); root.querySelector('#le-saved').textContent = savedLine(c);
...
async function saveNow(root, c) {
  ...
  busy(el, true);
  try { const wasNew = !c.a.id; await save(c); toast('Draft saved.', { ok: true }); if (wasNew) S.go('#/email/' + c.a.id, { replace: true, keepScroll: true }); else { busy(el, false); const s = root.querySelector('#le-saved'); if (s) s.textContent = savedLine(c); } }
  catch (e) { busy(el, false); toast(e, { err: true }); }
}
...
function wire(route, root) {
  ...
  if (src && !canEdit(src)) {
    on('[data-le="approve"]', async el => {
      ...
      busy(el, true);
      try { await DB.alertStep(src.id, 'approve'); hooks.render(); toast(`Approved. ${first(src.author_id)} can send it now.`, { ok: true }); }
      catch (e) { busy(el, false); toast(e, { err: true }); }
    });
