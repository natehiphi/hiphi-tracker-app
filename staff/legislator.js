...
// drawer. A compact header (under 120px on a phone), then Email · Call · Log a conversation, then the sections the
...
// introduced, team notes, their committees and contact details. Opened from a bill's Pathway, the back link reads
// "‹ HB1562" and returns to that Pathway, and that bill comes first.
import { S, DB, esc, hooks, fmtDate, advocate } from './data.js';
import { stopOf, stanceOf, legById, legsForSponsors, codesOf, diedish, hearingAhead, billNum, blurb, cmteName } from './model.js';
import { icon, btn, iconBtn, chip, empty, toast, openSheet, closeSheet } from './ui.js';
import { photo, legName, shortName, seatsOf, roleWord, legMail, billSub, billHref, stanceChip, wireStances, wireLinks } from './pathway.js';
import { ourBill } from './legislators.js';
...
// A bill row with the stance chip: the bill part links to the bill, the chip opens the stance sheet.
function standRow(b, l, { from = false } = {}) {
  const x = stanceOf(b.id, l.id), who = x.contact_id && advocate(x.contact_id);
  const sub = [billSub(b), who ? `${who.full_name.split(' ')[0]} knows them` : ''].filter(Boolean).join(' · ');
  return `<div class="row lg-brow${from ? ' from' : ''}">
    <a class="lg-bbody" href="${billHref(b)}"><span class="title"><b>${esc(billNum(b))}</b> <span class="lg-bt">${esc(blurb(b, 120))}</span></span><span class="sub">${esc(sub)}</span>${x.note ? `<span class="lg-note">${esc(x.note)}</span>` : ''}</a>
    ${stanceChip(b, l)}
  </div>`;
}
const billLink = b => `<a class="row lg-brow lg-plain" href="${billHref(b)}"><span class="lg-bbody"><span class="title"><b>${esc(billNum(b))}</b> <span class="lg-bt">${esc(blurb(b, 120))}</span></span><span class="sub">${esc(billSub(b))}</span></span>${icon('chevron-right', { cls: 'chev' })}</a>`;
...
function notesHTML(l) {
  ...
  return `<div class="rows">${list.map(n => {
    ...
    return `<div class="row lg-noterow"><div class="body"><p class="lg-nhead"><b>${esc(a?.full_name || 'Someone')}</b> · ${esc(fmtDate(n.created_at, { month: 'short' }))}${b ? ` · <a href="${billHref(b)}">${esc(b.bill_number)}</a>` : ''}</p><p class="lg-nbody">${esc(n.body)}</p></div>${mine ? iconBtn('trash-2', 'Delete this note', { 'data-lgdelnote': n.id }) : ''}</div>`;
  }).join('')}</div>`;
}
...
export default {
  ...
  render(route) {
    ...
    const from = fromBill(route), back = this.back(route);
    ...
    const stand = [...(from ? [standRow(from, l, { from: true })] : []), ...known.map(b => standRow(b, l))];
    ...
    const seats = seatsOf(l), cm = seats.map(m => m.role === 'member' ? m.committee : `${roleWord(m.role)}, ${m.committee}`).join(' · ');
    const hasCall = !!l.phone, hasMail = !!l.email;
    return `<div class="lg-page">
      ...
      <div class="lg-acts">
        ${hasMail ? btn('Email', { kind: 'secondary', icon: 'mail', href: legMail(l, from), sm: true }) : btn('Email', { kind: 'secondary', icon: 'mail', sm: true, attrs: { disabled: true, title: 'No email address on file' } })}
        ${hasCall ? btn('Call', { kind: 'secondary', icon: 'phone', href: `tel:${l.phone}`, sm: true }) : btn('Call', { kind: 'secondary', icon: 'phone', sm: true, attrs: { disabled: true, title: 'No phone number on file' } })}
        ${btn('Log a conversation', { sm: true, attrs: { 'data-lglog': '1' } })}
      </div>
      ${section('lg-s1', 'Where they stand on our bills', null, stand.length ? `<div class="rows">${stand.join('')}</div>` : `<p class="lg-empty">No stances yet. Set one on any bill below.</p>`)}
      ${section('lg-s2', 'Our bills in their committees', inCm.length, inCm.length ? `<div class="rows">${folded(`${l.id}|cm`, inCm.map(b => standRow(b, l)))}</div>` : `<p class="lg-empty">None of our live bills is in their committees right now.</p>`)}
      ${section('lg-s3', 'Bills they introduced', intro.length, intro.length ? `<div class="rows">${folded(`${l.id}|in`, intro.map(billLink))}</div>` : `<p class="lg-empty">None of the bills on the tracker.</p>`)}
      ${section('lg-s4', 'Team notes', null, notesHTML(l), btn('Add', { kind: 'text', icon: 'notebook-pen', sm: true, attrs: { 'data-lglog': '1', 'aria-label': 'Log a conversation' } }))}
      ${section('lg-s5', 'Committees', seats.length, seats.length ? `<div class="rows">${seats.map(m => `<div class="row lg-crow"><span class="body"><span class="title">${esc(cmteName(m.committee))}</span><span class="sub">${esc(m.committee)} · ${S.committees?.[m.committee]?.chamber === 'S' ? 'Senate' : 'House'}</span></span>${m.role !== 'member' ? chip(roleWord(m.role)) : ''}</div>`).join('')}</div>` : `<p class="lg-empty">No committee seats this session.</p>`)}
...
function logSheet(l, from) {
  ...
  const opt = b => `<option value="${esc(b.id)}"${from && b.id === from.id ? ' selected' : ''}>${esc(b.bill_number)} · ${esc(blurb(b, 50))}</option>`;
  let kind = '';
  openSheet({
