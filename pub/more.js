...
//   #/more      a short page of rows: your legislators, how it works, saving your progress (or Settings and Sign
//               out), HIPHI itself with the rest of hiphi.org folded away, privacy and accessibility
//   #/help      how it works in 3 steps, the words a newcomer meets at the Capitol, where to get a hand
//   #/signin    "Save your bills and actions": one email field, two opt-in email choices, privacy in 3 bullets
...
import { S, DEMO, app, esc, icon, ICONS, toast, friendly, yay, supa, fetchAddrSuggest, looksLikeAddress, legTitle,
  CONSENT_KEY, LOCAL_KEY, DONE_KEY, DONE_AT_KEY, LISTS_KEY, fmtDate } from './core.js';
...
const DISTRICTS_KEY = 'hiphi_districts';   // shared with the people and bill screens: { senate, house, label }
...
// The two email choices, worded once and used on sign in, the card after sign in, and Settings.
const CHOICES = [
  ['alerts', 'hearing_alerts', 'Email me when one of my bills gets a hearing', 'About 2 days’ notice, time to send testimony.'],
  ['action', 'action_alerts', 'Email me when HIPHI asks for help on my bills', 'A few times a session.'],
];
...
function rememberDistricts(sd, hd, address) {
  if (!sd || !hd) return;
  try { localStorage.setItem(DISTRICTS_KEY, JSON.stringify({ senate: +sd, house: +hd, label: splitAddr(address).town })); } catch { /* private mode */ }
}
...
export function accountCardsHTML() {
  ...
  if (S.consentCard) return `<section class="card mr-acct" id="mr-cc" aria-labelledby="mr-cc-t">
    ${acctHead('bell', 'mr-cc-t', 'Want a heads-up by email?')}
    <p class="small">Mahalo for signing in. Both are off unless you tick them.</p>
    <div class="mr-checks">${CHOICES.map(([k, , t, h]) => check(`mr-cc-${k}`, t, h, false)).join('')}</div>
    <p class="small muted">HIPHI staff can see the bills you follow and what you do here, so they can reach out about them. Change this any time in Settings.</p>
...
function moreView() {
  ...
  const legSub = s && h ? `${esc(legName(s))} and ${esc(legName(h))}` : 'Your senator and representative';
  const account = signedIn()
    ? row({ lead: 'settings', title: 'Settings', sub: esc(S.session.user.email || ''), href: '#/settings' })
      + row({ lead: 'log-out', title: 'Sign out', chevron: false, attrs: { 'data-mr-signout': '' } })
    : row({ lead: 'log-in', title: 'Save my progress', sub: 'Sign in with your email. No password.', href: '#/signin' });
  return `<div class="mr mr-more">
    <header class="pagehead"><h1>More</h1></header>
    <nav class="rows" aria-label="Tracker">
      ${row({ lead: 'users', title: 'Your legislators', sub: legSub, href: '#/legislators' })}
      ${row({ lead: 'circle-help', title: 'How it works', href: '#/help' })}
      ${account}
    </nav>
    <nav class="rows" aria-label="Hawaiʻi Public Health Institute">
      ${extRow('', 'About HIPHI', 'Hawaiʻi Public Health Institute', HIPHI.about, `<span class="lead mr-mark">${MARK}</span>`)}
      <details class="mr-fold"${S.mrFold ? ' open' : ''}>
        ...
        <div class="mr-foldb">
          ${extRow('megaphone', 'Action Center', 'More ways to speak up for health', HIPHI.act)}
    ...
    <nav class="rows" aria-label="About this tracker">
      ${row({ lead: 'lock', title: 'Privacy', href: '#/privacy' })}
      ${row({ lead: A11Y_ICON, title: 'Accessibility', href: '#/privacy?part=access' })}
    </nav>
...
function wireMore() {
  ...
  if (out) out.onclick = async () => {
    ...
    // Signing out reloads the page's data (core's onAuthStateChange). What was saved on this phone stays here.
    S.session = null; S.user = null; app.render();
    toast('You’re signed out. Your bills stay on this phone.');
  };
}

// ---------------- Help ----------------
const STEPS = [
  ['Follow bills you care about', 'Pick an issue, and HIPHI suggests bills worth your time. Tap the star to follow one.'],
  ['Watch for a hearing', 'A committee hears a bill before it votes. When one of your bills gets a hearing, it shows up on Home with the deadline.'],
  ['Speak up in about 5 minutes', 'Send testimony, a short letter to the committee. We help you write it. You can also email the chair or share the bill.'],
];
...
function helpView() {
  ...
  return `<div class="mr mr-help">
    <header class="pagehead"><h1>How it works</h1>
    ...
    <section aria-label="How it works in 3 steps">
    ...
    ${keys ? `<section aria-labelledby="mr-keys-t">
      ...
      <div class="card mr-keys">
        <div><kbd>/</kbd><span>Search bills</span></div>
...
// ---------------- Sign in ----------------
// M: this visit's sign-in page. The typed email and ticks survive a re-render; a fresh arrival starts clean.
const M = { email: '', choices: {}, sent: '', err: '', sendErr: '' };
const PRIVACY_BULLETS = [
  'HIPHI staff can see the bills you follow and the actions you mark, so they can reach out about them.',
...
function signinView() {
  if (S.session) return `<div class="mr mr-signin">
    <header class="pagehead"><h1>You’re signed in</h1>
      <p class="lede">You’re signed in as <span class="strong mr-break">${esc(S.session.user.email || '')}</span>. Your bills and actions are saved to your account.</p></header>
    <div class="btncol mr-narrow">${btn('Go to Settings', { kind: 'primary', icon: 'settings', href: '#/settings' })}${btn('Back to Home', { kind: 'text', href: '#/' })}</div>
  </div>`;
  if (M.sent) return `<div class="mr mr-signin">
    <header class="pagehead"><h1>Save your bills and actions</h1></header>
    <section class="card mr-sent" aria-labelledby="mr-sent-t">
      <span class="mr-sent-ic">${icon('mail-check')}</span>
      <h2 id="mr-sent-t" tabindex="-1">Check your inbox</h2>
      ${DEMO ? `<p>This is the sandbox, so no email was sent. On the real tracker, a sign-in link goes to <span class="strong mr-break">${esc(M.sent)}</span>.</p>`
        : `<p>We sent a sign-in link to <span class="strong mr-break">${esc(M.sent)}</span>. Open it on this phone to finish.</p>
      <p class="small muted">It can take a minute. If you don’t see it, check your spam folder.</p>`}
      <div id="mr-again-msg"></div>
      <div class="btnrow">${DEMO ? '' : btn('Send it again', { kind: 'text', sm: true, icon: 'rotate-ccw', attrs: { 'data-mr-again': '' } })}${btn('Use a different email', { kind: 'text', sm: true, attrs: { 'data-mr-other': '' } })}</div>
    </section>
  </div>`;
  return `<div class="mr mr-signin">
    <header class="pagehead"><h1>Save your bills and actions</h1>
      <p class="lede">We’ll email you a link. No password. Then your bills and actions follow you to any phone.</p></header>
    ${DEMO ? notice('info', 'info', 'Sign-in is off in the sandbox, so no email is sent.') : ''}
    <form class="card mr-form" id="mr-si" novalidate>
      ...
      <fieldset class="mr-set"><legend>Emails from HIPHI</legend>
        <p class="small muted">Optional. Both are off unless you tick them.</p>
        ${CHOICES.map(([k, , t, h]) => check(`mr-si-${k}`, t, h, !!M.choices[k])).join('')}
      </fieldset>
      <div id="mr-si-msg">${M.sendErr ? inlineErr('mr-si-err', M.sendErr) : ''}</div>
      ${submitBtn('Email me a sign-in link', 'mail', 'mr-si-send')}
    </form>
    <section class="mr-priv" aria-labelledby="mr-priv-t">
      ...
      <details class="mr-disc"><summary><span>More about privacy</span>${icon('chevron-down', { cls: 'mr-discc' })}</summary>
        <div class="mr-discb">
          <p>We keep your email, the bills and lists you follow, the actions you mark, your email choices, and anything you add in Settings: your name, phone, address and how you’d like to help.</p>
          <p>When you sign in, the bills and actions saved on this phone join your account.</p>
...
async function sendLink(email) {
  return S.supa.auth.signInWithOtp({ email, options: { emailRedirectTo: location.origin + location.pathname } });
}
function wireSignin() {
  ...
  if (again) again.onclick = async () => {
    busy(again, 'Sending…');
    const { error } = await sendLink(M.sent);
    unbusy(again);
  ...
  inp.oninput = () => { M.email = inp.value; if (M.err && EMAIL_RE.test(inp.value.trim())) setErr(''); };
  inp.onblur = () => { const v = inp.value.trim(); if (v && !EMAIL_RE.test(v)) setErr(ERR); };
  CHOICES.forEach(([k]) => { const c = $(`#mr-si-${k}`); if (c) c.onchange = () => { M.choices[k] = c.checked; }; });
  form.onsubmit = async e => {
    e.preventDefault();
    const email = inp.value.trim(); M.email = email; M.sendErr = ''; say('mr-si-msg', '');
    if (!EMAIL_RE.test(email)) { setErr(ERR); focusField(inp); return; }
    setErr('');
    if (DEMO) { M.sent = email; app.render(); requestAnimationFrame(() => $('#mr-sent-t')?.focus()); return; }
    // The account does not exist until the link is opened, so the choices wait on this device; core's loadUser
    // saves them to the account at the first sign-in (and records when they were made).
    try { localStorage.setItem(CONSENT_KEY, JSON.stringify({ hearing_alerts: !!$('#mr-si-alerts')?.checked, action_alerts: !!$('#mr-si-action')?.checked })); } catch { /* private mode */ }
    ...
    M.sent = email; app.render(); requestAnimationFrame(() => $('#mr-sent-t')?.focus());
  };
}
...
function settingsView() {
  if (!S.session) return `<div class="mr mr-settings">
    <header class="pagehead"><h1>Settings</h1>
      <p class="lede">${DEMO ? 'Settings appear here once someone signs in. Sign-in is off in the sandbox.' : 'Sign in to choose your emails and add your details.'}</p></header>
    ${btn('Sign in', { kind: 'primary', icon: 'log-in', href: '#/signin' })}
  </div>`;
  if (!S.user) return `<div class="mr mr-settings"><header class="pagehead"><h1>Settings</h1></header><div class="skelpage" aria-busy="true" aria-label="Loading"><div class="skel" style="height:160px"></div><div class="skel" style="height:280px"></div></div></div>`;
  if (!F) freshForm();
  const p = S.user.prefs || {};
  return `<div class="mr mr-settings">
    <header class="pagehead"><h1>Settings</h1><p class="meta mr-break">Signed in as ${esc(S.session.user.email || '')}</p></header>
    ...
    <section aria-labelledby="mr-data-t">
      <h2 id="mr-data-t">Your data</h2>
      <p>We keep your email, the bills and lists you follow, the actions you mark, your email choices and what you add above. HIPHI staff can see all of it, except your street address: they see just your districts. We never sell it or share it outside HIPHI. <a href="#/privacy">Read about privacy</a></p>
      ${F.del ? `<div class="mr-confirm" role="group" aria-labelledby="mr-del-t">
          ${notice('bad', 'triangle-alert', `<p class="strong" id="mr-del-t" tabindex="-1">Delete your account for good?</p><p>This deletes your account, the bills you follow and the actions you marked, here and on our side. It can’t be undone.</p>`)}
...
function wireSettings() {
  ...
  if (yes) yes.onclick = async () => {
    ...
    // The account is gone; the copies of it on this phone go too (bills, actions, list follows, districts).
    try { [LOCAL_KEY, DONE_KEY, DONE_AT_KEY, LISTS_KEY, CONSENT_KEY, DISTRICTS_KEY].forEach(k => localStorage.removeItem(k)); } catch { /* private mode */ }
    S.watch = new Set(); S.done = new Set(); S.doneAt = {}; S.listFollows = new Set(); S.profile = {}; F = null;
...
const PRIVACY = [
  ['lock', 'If you don’t sign in', 'HIPHI gets nothing about you. The bills you follow and the actions you mark stay in this browser, on this phone. Clearing your browser data erases them.'],
  ['user', 'If you sign in', 'We keep your email, the bills and lists you follow, the actions you mark, your email choices, and anything you add in Settings. HIPHI staff can see this, so they can reach out about your bills. What you saved on this phone joins your account.'],
  ['map-pin', 'Your home address', 'If you add it in Settings, only you see it. HIPHI staff see just the districts it falls in. When you look up your legislators, the address you type is used for that search only. It isn’t saved.'],
  ['notebook-pen', 'Your testimony', 'Your letter stays on this phone until you send it on the Capitol website. Testimony is public there: the Capitol posts your name and letter online.'],
...
function privacyView() {
  return `<div class="mr mr-privacy">
    <header class="pagehead"><h1>Privacy</h1><p class="lede">What we keep, who sees it, and how to remove it.</p></header>
    ...
    <section class="mr-access" id="mr-access" aria-labelledby="mr-access-t">
      <h2 id="mr-access-t" tabindex="-1">Accessibility</h2>
      <p>We want everyone in Hawaiʻi to be able to use this tracker. It is built to work with screen readers, a keyboard and zoom. Buttons are large enough to tap, and motion turns off when your phone asks for less of it.</p>
...
const TITLES = { more: 'More', help: 'How it works', signin: 'Save your bills', settings: 'Settings', privacy: 'Privacy' };
export default {
  tab: 'more',
