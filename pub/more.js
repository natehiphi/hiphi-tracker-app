...
//   #/signin    "Add your email": one email field, hearing alerts on, HIPHI's action alerts off, privacy in 3 bullets
...
// header's word for returning people, plus one small line on that page. Giving an email for hearing alerts IS the
// consent for them, so that box starts ticked here; HIPHI's own action alerts stay a separate choice, off.
...
import { S, DEMO, app, esc, icon, ICONS, toast, friendly, yay, supa, fetchAddrSuggest, looksLikeAddress, legTitle,
  CONSENT_KEY, LOCAL_KEY, DONE_KEY, DONE_AT_KEY, LISTS_KEY, STANCE_KEY, fmtDate, sendEmailLink, validEmail } from './core.js';
...
// The two email choices, worded once and used on Add your email, the card after sign in, and Settings.
// The first is what the email is for, so it starts ticked wherever a person is giving their email (Nate, 9/19);
// the second is HIPHI's own ask and is never ticked for anyone.
const CHOICES = [
  ['alerts', 'hearing_alerts', 'Email me when one of my bills gets a hearing', 'About 2 days’ notice, time to send testimony.'],
  ['action', 'action_alerts', 'Also email me when HIPHI asks for help on my bills', 'A few times a session. Off unless you tick it.'],
];
...
export function accountCardsHTML() {
  ...
  if (S.consentCard) return `<section class="card mr-acct" id="mr-cc" aria-labelledby="mr-cc-t">
    ...
    <p class="small">Mahalo for adding your email. Hearing alerts are ticked for you. Nothing is sent until you save.</p>
    <div class="mr-checks">${CHOICES.map(([k, , t, h]) => check(`mr-cc-${k}`, t, h, k === 'alerts')).join('')}</div>
    <p class="small muted">HIPHI staff can see the bills you follow, where you stand on them and what you do here, so they can reach out about them. Change this any time in Settings.</p>
...
function moreView() {
  ...
  const account = signedIn()
    ...
    : row({ lead: 'mail-check', title: 'Add my email', sub: 'Get hearing alerts and keep your bills on any device. No password.', href: '#/signin' });
...
// ---------------- Help ----------------
// The first visit is small on purpose (Nate, 9/19): pick issues, follow a few bills, say where you stand. Ways to
// help come on later visits, easiest first. Nothing here promises community totals: progress is the person's own.
const STEPS = [
  ['Pick what you care about', 'Choose an issue or two, like keiki health or vaping. HIPHI suggests a few bills worth your time.'],
  ['Follow a few bills', 'Select the star on a bill to follow it. It goes to My bills, where you can see what happens to it.'],
  ...
  ['Speak up when it counts', 'When one of your bills gets a hearing, Home shows ways to help, easiest first: a two-minute email to the chair, then testimony, a short letter we help you write.'],
];
...
// M: this visit's page. The typed email and ticks survive a re-render; a fresh arrival starts clean. Hearing alerts
// start ticked: the page says what the email is for, so giving it is the consent (Nate, 9/19). HIPHI's own action
// alerts are a separate choice and start unticked.
const M = { email: '', choices: { alerts: true, action: false }, sent: '', demo: false, err: '', sendErr: '' };
const PRIVACY_BULLETS = [
  'HIPHI staff can see the bills you follow, where you stand on them and the actions you mark, so they can reach out about them.',
...
function signinView() {
  if (S.session) return `<div class="mr mr-signin">
    <header class="pagehead"><h1 class="hero">You’re signed in</h1>
      <p class="lede">Your email is <span class="strong mr-break">${esc(S.session.user.email || '')}</span>. Your bills, stances and actions are saved to your account.</p></header>
  ...
  return `<div class="mr mr-signin">
    <header class="pagehead"><h1 class="hero">Add your email</h1>
      <p class="lede">Get hearing alerts and keep your bills on any device. No password: we email you a link.</p></header>
    ...
    <form class="card mr-form mr-panel" id="mr-si" novalidate>
      ...
      <fieldset class="mr-set"><legend>What we’ll email you</legend>
        <p class="small muted">Hearing alerts are ticked for you. Untick the box if you only want to keep your bills on any device.</p>
    ...
    <section class="mr-priv mr-panel" aria-labelledby="mr-priv-t">
      ...
      <details class="mr-disc"><summary><span>More about privacy</span>${icon('chevron-down', { cls: 'mr-discc' })}</summary>
        <div class="mr-discb">
          <p>We keep your email, the issues you picked, the bills and lists you follow, where you stand on the bills you follow, the actions you mark, your email choices, and anything you add in Settings: your name, phone, address and how you’d like to help.</p>
          <p>When you open the link, the bills, stances and actions saved on this device join your account.</p>
...
function wireSignin() {
  ...
  form.onsubmit = async e => {
    ...
    try { r = await sendEmailLink(email, { hearing_alerts: !!$('#mr-si-alerts')?.checked, action_alerts: !!$('#mr-si-action')?.checked }); } catch (err) { error = err; }
    inp.readOnly = false;
  ...
  return `<div class="mr mr-settings">
    ...
    <section class="mr-data mr-panel" aria-labelledby="mr-data-t">
      ...
      ${F.del ? `<div class="mr-confirm" role="group" aria-labelledby="mr-del-t">
          ${notice('bad', 'triangle-alert', `<p class="strong" id="mr-del-t" tabindex="-1">Delete your account for good?</p><p>This deletes your account, the bills you follow, where you stand on them and the actions you marked, here and on our side. It can’t be undone.</p>`)}
...
function wireSettings() {
  ...
  if (yes) yes.onclick = async () => {
    ...
    // The account is gone; the copies of it on this device go too (bills, stances, actions, list follows, districts,
    // and the email the testimony helper remembered; the name, town and letters typed there were never on the account).
    try { [LOCAL_KEY, STANCE_KEY, DONE_KEY, DONE_AT_KEY, LISTS_KEY, CONSENT_KEY, DISTRICTS_KEY].forEach(k => localStorage.removeItem(k));
    ...
    S.watch = new Set(); S.stances = {}; S.done = new Set(); S.doneAt = {}; S.listFollows = new Set(); S.profile = {}; F = null;
...
const PRIVACY = [
  ['lock', 'If you don’t add your email', 'We don’t know who you are. The bills you follow, where you stand on them and the actions you mark stay in this browser, on this device. Clearing your browser data erases them.'],
  ['user', 'If you add your email', 'We keep your email, the issues you picked, the bills and lists you follow, where you stand on each bill you follow (support, oppose or not sure), the actions you mark, your email choices, and anything you add in Settings. HIPHI staff can see this, so they can reach out about your bills. What you saved on this device joins your account.'],
