...
export function btn(label, { kind = 'primary', icon: ic, iconEnd, full, sm, href, attrs: a, cls = '' } = {}) {
  ...
  return href ? `<a class="${c}" href="${esc(href)}"${attrs(a)}>${inner}</a>` : `<button type="button" class="${c}"${attrs(a)}>${inner}</button>`;
}
...
export const empty = ({ art = '', title, text = '', action = '' }) => `<div class="empty">${art ? `<div class="art">${art}</div>` : ''}${title ? `<h3>${title}</h3>` : ''}${text ? `<p>${text}</p>` : ''}${action}</div>`;
